...
def create_path(s):
    ...
    try:
        ...
        assert False, "Creation of the dir %s failed *The TEMP folder may already exist. Delete/rename it, n try again."
...
FRAME_SPREAD_AGE = args.frame_margin
...
for i in range(audioFrameCount):
    ...
    audio_chunks = audioData[start:end]
    max_chunks_volume = float(get_max_volume(audio_chunks)) / maxAudioVolume
...
for i in range(audioFrameCount):
    start = int(max(0, i - FRAME_SPREAD_AGE))
    end = int(min(audioFrameCount, i + 1 + FRAME_SPREAD_AGE))
